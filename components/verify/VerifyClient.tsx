...
export default function VerifyClient({ incidentId }: { incidentId: string }) {
  ...
  useEffect(() => {
    ...
    const fetchToken = async (): Promise<TokenResp> => {
      const r = await fetch(`${process.env.NEXT_PUBLIC_API_BASE!}/Prod/sumsub/token`, {
        method: "POST",
  ...
  const onContinue = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE!}/Prod/incidents/${encodeURIComponent(incidentId)}/finalize`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
