...
export async function submitIncident(
  ...
  const endpoint = `${process.env.NEXT_PUBLIC_API_BASE_URL}/Prod/incidents_create`;
  const bodyToSend: Record<string, any> = {
    ...payload,
    ...(verificationData?.verificationId && {
      sumSubVerificationId: verificationData.verificationId,
    }),
  };
  ...
  return {
    caseId: data.incidentId || `CASE-SIM-${Date.now().toString().slice(-5)}`,
  };
}
